        subroutine setstrmln (error)
c-----------------------------------------------------------------------
c     $Id: setstrmln.f,v 1.1.1.1 1993/04/07 19:40:33 harry Exp $
c     Purpose:
c        This routine sets the streamline flag. If this flag is on,
c        streamlines are drawn. Note that the field must be a vector
c        field.
c     Arguments:
...
c
c Revision 2.0  1992/07/01  21:14:56  millerp
...
c
c           Turn streamline flag on.
c
            drstrmln = .true.
c
         elseif (curval(ibeg:iend) .eq. 'OFF') then
c
c           Turn streamline flag off.
c
            drstrmln = .false.
c
         else
