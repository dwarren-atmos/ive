      subroutine setabscissa (error)
c-----------------------------------------------------------------------
c     $Id: setabscissa.f,v 1.15 1997/06/12 15:28:18 davidive Exp $
c     Purpose:
c        This routine sets the upper and lower limits of the horizontal
...
c
c Revision 2.0  1992/07/01  21:14:03  millerp
...
c
 ...
 10   continue
c
c     Set the abscissa min and/or max, as appropriate. Also set flags
c     indicating whether or not the user has specified these values.
c
      if ( .not. error ) then
         if ( value(1) .ne. FILLVAL ) then
            abscissa_min = value(1)
         endif
         if ( value(2) .ne. FILLVAL ) then
            abscissa_max = value(2)
         endif
      endif
c
      if (lock(3) .eq. 0 .or. use_buttons .eq. 0) then
         lmin = abscissa_min
         lmax = abscissa_max
      endif
c
